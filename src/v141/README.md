...
| 系统 | 文件 | 安装方法 |
|---|---|---|
| Mac（Apple 芯片 M1–M4） | `DocDiff-1.4.1-mac-arm64.dmg` | 双击打开，把 DocDiff 拖到右边的"应用程序" |
| Mac（Intel 芯片） | `DocDiff-1.4.1-mac-x64.dmg` | 同上 |
| Windows 10 / 11（64 位） | `DocDiff-Setup-1.4.1.exe` | 双击安装，可选择安装位置，会创建桌面和开始菜单快捷方式 |
| Windows 免安装版 | `DocDiff-1.4.1-win-x64.zip` | 解压后运行 `DocDiff.exe` |
