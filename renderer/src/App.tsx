...
function textFile(text: string, side: Side): FileInfo {
  ...
  const now = Date.now();
  return { path: '', name: `粘贴的文字（${side ? '修改后' : '原始'}）.txt`, ext: 'txt', size: data.length, mtime: now, birthtime: now, data };
}
const docStat = (d: DocModel) => `${d.type.toUpperCase()} · ${fmtBytes(d.file.size)}${d.pageCount ? ` · ${d.pageCount} 页` : ''}`;
...
function DropZone({ side, doc, loading, error, onOpen, onFiles, onClear, onPaste }: {
  ...
  return (
    <div className={`drop ${d.over ? 'over' : ''} ${doc ? 'filled' : ''}`} {...d.props}>
      <div className="label">{SIDE_LABEL[side]}</div>
      {loading ? (<><div className="spinner" /><div className="hint">正在读取…</div></>) : doc ? (
        <>
          <I.FileGlyph type={doc.type} size={64} />
          <div className="name">{doc.file.name}</div>
          <div className="sub">{docStat(doc)} · {doc.blocks.filter((b) => b.kind === 'tr' || b.runs.length).length} 段</div>
          {doc.type === 'pdf' && doc.blocks.length === 0 && <div className="err">没读到文字，可能是扫描件。比较后用「OCR 文本」或「图像」模式。</div>}
          <div className="acts"><button className="btn sm" onClick={onOpen}><I.IconUpload />更换</button><button className="btn sm plain" onClick={onClear}>移除</button></div>
        </>
      ) : (
        <>
          <div style={{ opacity: .55 }}><I.FileGlyph type="txt" size={60} /></div>
          <div className="title">拖入{SIDE_LABEL[side]}</div>
          <div className="hint">支持 Word（.docx）、PDF、TXT</div>
          <div className="acts">
            <button className="btn blue" onClick={onOpen}><I.IconUpload />选择文件</button>
            <button className="btn" onClick={onPaste}><I.IconPaste />粘贴文字</button>
          </div>
        </>
      )}
...
function FilePill({ side, doc, onOpen, onFiles, changed }: { side: Side; doc: DocModel; onOpen: () => void; onFiles: (f: FileInfo[]) => void; changed: boolean }) {
  ...
  return (
    <div className={`filepill ${d.over ? 'over' : ''}`} {...d.props} title={doc.file.path || doc.file.name}>
      ...
      <div className="grow">
        <span className="who">{SIDE_LABEL[side]}{changed ? ' · 已更新' : ''}</span>
        <span className="name">{doc.file.name}</span>
      </div>
      <button className="btn sm plain icon" onClick={onOpen} title="换一个文件（也可以直接拖进来）"><I.IconUpload /></button>
    </div>
  );
}
  ...
  return (
    <div className="modal-bg" onClick={onCancel}>
      <div className="modal" onClick={(e) => e.stopPropagation()}>
        <h3>这个 PDF 有密码</h3>
        <p>「{name}」受密码保护。{wrong && <b style={{ color: 'var(--cardinal)' }}>密码不对，再试一次。</b>}</p>
        <input type="password" autoFocus value={v} onChange={(e) => setV(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && onOk(v)} placeholder="输入密码" />
        <div className="acts"><button className="btn plain" onClick={onCancel}>取消</button><button className="btn primary" onClick={() => onOk(v)}>打开</button></div>
      </div>
    </div>
  );
}
  ...
  return (
    <div className="modal-bg" onClick={onCancel}>
      <div className="modal wide" onClick={(e) => e.stopPropagation()}>
        <h3>粘贴{SIDE_LABEL[side]}的文字</h3>
        <p>直接粘贴从网页、邮件、聊天软件里复制的内容。</p>
        <textarea autoFocus value={v} onChange={(e) => setV(e.target.value)} placeholder={`在这里粘贴…（${kbd('mod+V')}）`} />
        <div className="acts"><button className="btn plain" onClick={onCancel}>取消</button><button className="btn primary" disabled={!v.trim()} onClick={() => onOk(v)}>使用这段文字</button></div>
      </div>
    </div>
  );
}
...
function ShortcutsModal({ onClose }: { onClose: () => void }) {
  const rows: [string, string][] = [
    [kbd('mod+O'), '打开原始文档'], [kbd('mod+shift+O'), '打开修改后文档'], [kbd('mod+N'), '新建比较'],
    [`${kbd('mod+1')} … ${kbd('mod+6')}`, '切换模式'], ['J / K', '下一处 / 上一处差异'],
    ['A / R', '（修订审阅）接受 / 拒绝'], ['U', '（修订审阅）撤销当前决定'], [kbd('mod+shift+S'), '交换左右文档'], ['Enter', '（首页）开始比较'],
  ];
  return (
    <div className="modal-bg" onClick={onClose}>
      <div className="modal" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="acts"><button className="btn primary" onClick={onClose}>知道了</button></div>
      </div>
    </div>
  );
}
...
export default function App() {
  ...
  const richRes = useMemo(() => (compared && A && B ? diffDocs(A.blocks, B.blocks, { ...opts, mergeParas: true }) : null), [compared, A, B, optKey]);
  const plainBlocks = useMemo(() => (A && B ? [linesToBlocks(docPlainText(A)), linesToBlocks(docPlainText(B))] : null), [A, B]);
  const plainRes = useMemo(() => (compared && plainBlocks ? diffDocs(plainBlocks[0], plainBlocks[1], { ...opts, formatting: false, keepEmpty: true }) : null), [compared, plainBlocks, optKey]);
  ...
  const ocrRes = useMemo(() => (ocrBlocks ? diffDocs(ocrBlocks[0], ocrBlocks[1], { ...opts, formatting: false, keepEmpty: true }) : null), [ocrBlocks, optKey]);
  ...
  const exportDocx = async (finalMode: boolean) => {
    ...
    const p = await api.saveFile({ defaultName: `${baseName}-${finalMode ? '最终稿' : '修订稿'}.docx`, data: bytes, filters: [{ name: 'Word 文档', extensions: ['docx'] }] });
    if (p) flash(`已导出：${p}`);
  };
  const exportPdf = async () => {
    ...
    </style></head><body><div class="head-note">修订对比：${A!.file.name} → ${B!.file.name}　·　红色删除线 = 删除，绿色下划线 = 新增，紫色 = 移动，橙色底线 = 格式变化</div>${clone.outerHTML}</body></html>`;
    const p = await api.htmlToPdf({ html, defaultName: `${baseName}-修订对比.pdf` });
    if (p) flash(`已导出：${p}`);
  };
  const copySummary = () => {
    if (!curRes || !A || !B) return;
    api.copyText(changesMarkdown(curRes.changes, A.file.name, B.file.name, curRes.stats));
    flash(`已复制 ${curRes.changes.length} 处差异的摘要，可以直接粘贴`);
  };
  ...
  const runOcr = async () => {
    ...
    setOcr({ status: 'running', msg: '正在准备页面…', pct: 0, key });
    let off: (() => void) | null = null;
    try {
      const texts: string[] = [];
      for (const [si, d] of [[0, A], [1, B]] as [number, DocModel][]) {
        const pages = await getPages(d, 2, (i, n) => setOcr((o) => ({ ...o, msg: `正在准备${SIDE_LABEL[si]}的第 ${i}/${n} 页` })));
        ...
        off = api.onOcrProgress(({ page, progress }: any) => {
          ...
          setOcr((o) => (o.status === 'running' ? { ...o, pct, msg: `正在识别${SIDE_LABEL[si]}：第 ${page + 1}/${total} 页` } : o));
        });
        setOcr((o) => ({ ...o, msg: '正在加载识别模型…' }));
        const res: string[] = await api.ocr({ images: imgs, langs });
        texts.push(res.map((t, i) => (res.length > 1 ? `—— 第 ${i + 1} 页 ——\n` : '') + cleanOcr(t)).join('\n\n'));
      }
      setOcr({ status: 'done', a: texts[0], b: texts[1], key });
      api.notify?.('OCR 识别完成', `${A.file.name} 与 ${B.file.name} 已可以比较`);
  ...
  const cancelOcr = () => { api.ocrCancel?.(); setOcr({ status: 'idle' }); flash('已取消识别'); };
  ...
  actions.current = { open, swap, clearAll, go, decide, setTab, startCompare, compared, tab, selCid, decisions, modal: !!(pwd || pasteSide !== null || showKeys) };
  ...
  const themeBtn = <button className="btn plain icon" title={theme === 'dark' ? '浅色模式' : '深色模式'} onClick={() => setTheme(theme === 'dark' ? 'light' : 'dark')}>{theme === 'dark' ? <I.IconSun /> : <I.IconMoon />}</button>;
  const modals = (
    <>
  ...
  if (!compared || !A || !B) {
    const openRecent = async (r: any) => {
      ...
      if (!ok[0] || !ok[1]) { flash(`找不到「${!ok[0] ? r.a.name : r.b.name}」，它可能被移动或删除了`); return; }
    ...
    return (
      <div className="app home">
        <div className="home-top drag">
          <Wordmark size={36} sub="离线文档比对" />
          <div className="spacer" />
          <button className="btn plain icon" title="快捷键" onClick={() => setShowKeys(true)}><I.IconKeyboard /></button>
          {themeBtn}
        </div>
        <div className="home-scroll">
          <div className="home-inner">
            <div className="hero">
              ...
              <div>
                <h1>{A && B ? '准备好了！按「开始比较」吧' : '找出两份文档的每一处不同'}</h1>
                <p>Word、PDF、TXT 任意组合 · 所有处理都在这台电脑上完成，不会上传</p>
                <div className="pills">
                  {MODES.map((m) => <span key={m.label} className="pill">{m.icon}{m.label}</span>)}
                </div>
              </div>
            </div>
            <div className="drops">
              ...
              <div className="vs"><button className="vs-chip" title="交换左右" onClick={swap}><I.IconSwap /></button></div>
                ...
                <div className="recent-grid">
                  {recent.map((r, i) => (
                    <div key={i} className="recent-item" role="button" tabIndex={0} onClick={() => openRecent(r)} onKeyDown={(e) => e.key === 'Enter' && openRecent(r)}>
                      ...
                      <button className="btn sm plain icon x" title="从列表移除" onClick={(e) => { e.stopPropagation(); api.recentRemove(i).then(setRecent); }}><I.IconX /></button>
                    </div>
                  ))}
                </div>
              </div>
        ...
        <div className="home-cta">
            {canCompare && <span className="hint">按 <Kbd>Enter</Kbd> 也可以</span>}
            <button className="btn primary big" disabled={!canCompare} onClick={startCompare}>{A && B ? '开始比较' : `还差 ${2 - (A ? 1 : 0) - (B ? 1 : 0)} 个文档`}</button>
          </div>
        {modals}
      </div>
    );
  ...
  const navBtns = changes.length > 0 && (
    <span style={{ display: 'inline-flex', gap: 8 }}>
      <button className="btn sm" onClick={() => go(-1)} title="上一处（K）"><I.IconUp />上一处</button>
      <button className="btn sm" onClick={() => go(1)} title="下一处（J）"><I.IconDown />下一处</button>
    </span>
  );

  // 中间工具条
  let tools: React.ReactNode = null;
  if (tab === 0) tools = (<><span className="legend"><span><span className="tag del">删除</span></span><span><span className="tag ins">新增</span></span><span><span className="tag fmt">格式</span></span><span><span className="tag move">移动</span></span></span><span className="grow" />{navBtns}</>);
  else if (tab === 1) tools = (<><Seg value={prefs.layout} options={[['split', '并排'], ['unified', '合并']]} onChange={set('layout')} /><span className="grow" />{navBtns}</>);
  else if (tab === 2) tools = (
    <>
      ...
      <div style={{ position: 'relative' }}>
        <button className="btn sm" onClick={() => setExportOpen((v) => !v)}><I.IconExport />导出</button>
        {exportOpen && (
          <div className="menu" style={{ right: 0, top: 40 }} onMouseLeave={() => setExportOpen(false)}>
            <button onClick={() => { setExportOpen(false); exportDocx(true); }}><b>Word 最终稿</b><small>还没处理的修订按"接受"算</small></button>
            <button onClick={() => { setExportOpen(false); exportDocx(false); }}><b>Word 修订稿</b><small>保留删除线和下划线标记</small></button>
            <button onClick={() => { setExportOpen(false); exportPdf(); }}><b>PDF</b><small>当前修订视图，适合发给别人看</small></button>
          </div>
        )}
      </div>
    </>
    ...
    <>
      <Seg value={prefs.imgMode} options={[['split', '并排'], ['diff', '差异'], ['fade', '淡化'], ['slider', '滑块']]} onChange={set('imgMode')} />
      {prefs.imgMode === 'fade' && <><span className="progress-label">原始</span><input type="range" className="range" style={{ width: 200 }} min={0} max={100} value={fade} onChange={(e) => setFade(Number(e.target.value))} /><span className="progress-label">修改后</span></>}
      {prefs.imgMode === 'slider' && <span className="progress-label">拖动页面上的分割线来对比</span>}
      <span className="grow" />
      <select className="sel" value={zoom} onChange={(e) => setZoom(Number(e.target.value))} title="缩放">
    ...
    <>
      <select className="sel" value={prefs.ocrLang} onChange={(e) => set('ocrLang')(e.target.value as OcrLang)} disabled={ocr.status === 'running'}>
        <option value="chi_sim+eng">中文 + 英文</option><option value="chi_sim">仅中文</option><option value="eng">仅英文</option>
      </select>
      {ocr.status === 'running' ? <button className="btn sm danger-outline" onClick={cancelOcr}><I.IconX />取消识别</button>
        : <button className="btn sm blue" onClick={runOcr}><I.IconOcr />{ocrRes ? '重新识别' : '开始识别'}</button>}
  ...
  if (tab === 0 && richRes) body = richRes.rows.length ? <RichView A={A} B={B} res={richRes} selCid={selCid} onSel={setSelCid} hideUnchanged={prefs.hideRich} />
    : <div className="empty-state"><Mascot mood="oops" /><h2>没找到可以比较的文字</h2><div>{(A.type === 'pdf' || B.type === 'pdf') ? '如果是扫描件，试试「OCR 文本」或「图像」模式。' : '两份文档好像都是空的。'}</div></div>;
    ...
    : <div className="empty-state"><Mascot mood="cheer" /><h2>没有需要审阅的修订</h2><div>两份文档内容一致。</div></div>;
    ...
    else if (ocr.status === 'running') body = <div className="empty-state"><Mascot mood="think" className="mascot-bob" /><h2>正在认真读字…</h2><div>{ocr.msg}</div><div className="progress"><div style={{ width: `${ocr.pct || 0}%` }} /></div><button className="btn sm plain" onClick={cancelOcr}>取消</button></div>;
    else body = (
      <div className="empty-state">
        <Mascot mood={ocr.status === 'error' ? 'oops' : 'happy'} />
        <h2>先识别，再比较</h2>
        <div>把两份文档的每一页当成图片来识别文字。适合扫描件、截图型 PDF。<br />每页大约 3–5 秒，完全离线。</div>
        {ocr.status === 'error' && <div className="err">识别失败：{ocr.msg}</div>}
        <button className="btn primary big" onClick={runOcr}>开始识别</button>
      </div>
    );
  ...
  const summaryCard = curRes && (
    <Card title="差异概览" right={changes.length > 0 && <button className="btn sm" onClick={copySummary} title="复制为 Markdown，可粘贴到邮件 / 飞书 / 文档"><I.IconCopy />复制摘要</button>}>
      {changes.length === 0 ? (
        <div className="same-banner"><Mascot mood="cheer" size={48} />内容完全一致！</div>
      ) : (
        <div className="stats">
          <div className="stat-tile del"><span className="ic">−</span><div><b>{stats!.del}</b><br /><span>删除</span></div></div>
          <div className="stat-tile ins"><span className="ic">+</span><div><b>{stats!.ins}</b><br /><span>新增</span></div></div>
          <div className="stat-tile mod"><span className="ic">✎</span><div><b>{stats!.mod}</b><br /><span>修改</span></div></div>
          <div className="stat-tile other"><span className="ic">⇅</span><div><b>{stats!.fmt + stats!.move}</b><br /><span>格式 / 移动</span></div></div>
        </div>
      )}
    </Card>
  );
  const compareSettings = (
    <>
      <Opt label="忽略空白差异" hint="多余空格、换行符不同"><Switch on={prefs.ignoreWs} onChange={set('ignoreWs')} /></Opt>
      <Opt label="忽略全角 / 半角标点" hint="，和 ,　（和 (　视为相同"><Switch on={prefs.normPunct} onChange={set('normPunct')} /></Opt>
      <Opt label="忽略大小写"><Switch on={prefs.ignoreCase} onChange={set('ignoreCase')} /></Opt>
      <Opt label="检测移动的段落"><Switch on={prefs.detectMoves} onChange={set('detectMoves')} /></Opt>
      {(tab === 0 || tab === 2) && <Opt label="检测格式变化" hint={A.type !== B.type ? 'Word 与 PDF 互比时默认关闭' : '加粗、字号、颜色、标题级别'}><Switch on={formatting} onChange={setFormatting} /></Opt>}
      {tab === 0 && <Opt label="只看有变化的段落"><Switch on={prefs.hideRich} onChange={set('hideRich')} /></Opt>}
      {(tab === 1 || tab === 4) && <><Opt label="只看有变化的行"><Switch on={prefs.hidePlain} onChange={set('hidePlain')} /></Opt><Opt label="自动换行"><Switch on={prefs.wrap} onChange={set('wrap')} /></Opt></>}
      <Opt label="比较精度"><Seg value={prefs.precision} options={[['word', '按词'], ['char', '按字符']]} onChange={set('precision')} /></Opt>
      {tab === 2 && <Opt label="音效"><Switch on={prefs.sound} onChange={set('sound')} /></Opt>}
    </>
  );
        ...
        {tab === 2 && changes.length > 0 && (
          <Card title="批量处理">
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
              <button className="btn sm" style={{ '--fg-b': 'var(--feather-dark)' } as any} onClick={() => decideAll('a')}><I.IconCheck />全部接受</button>
              <button className="btn sm danger-outline" onClick={() => decideAll('r')}><I.IconX />全部拒绝</button>
            </div>
            {decidedCount > 0 && <button className="btn sm plain block" style={{ marginTop: 8 }} onClick={() => decideAll(null)}>重置所有决定</button>}
          </Card>
        )}
        {changes.length > 0 && (
          <Card className="grow-card" title={<>{tab === 2 ? '修订列表' : '变更列表'} <span style={{ color: 'var(--text-3)', fontSize: 14 }}>{changes.length}</span></>}>
            <ChangeList changes={changes} sel={selCid} onSel={setSelCid} decisions={tab === 2 ? decisions : undefined} />
          </Card>
        )}
        <Card title={<span style={{ display: 'inline-flex', alignItems: 'center', gap: 8 }}><I.IconGear />比较设置</span>} collapsible open={settingsOpen} onToggle={() => setSettingsOpen((v) => !v)}>
      ...
      <>
        <Card title="版面对比">
          {imgState.loading && !imgState.diffs.length ? <div className="progress-label">{imgState.loading}</div> : diffPages ? (
            <div className="stats"><div className="stat-tile del"><span className="ic">!</span><div><b>{diffPages}</b><br /><span>页有差异</span></div></div><div className="stat-tile ins"><span className="ic">✓</span><div><b>{imgState.diffs.length - diffPages}</b><br /><span>页相同</span></div></div></div>
          ) : <div className="same-banner"><Mascot mood="cheer" size={48} />每一页都一模一样！</div>}
          <div style={{ marginTop: 10 }}>
            <Opt label="标出差异区域"><Switch on={prefs.showBoxes} onChange={set('showBoxes')} /></Opt>
            <Opt label="只看有差异的页"><Switch on={onlyDiffPages} onChange={setOnlyDiffPages} /></Opt>
            <Opt label="灵敏度">
              <select className="sel" value={prefs.threshold} onChange={(e) => set('threshold')(Number(e.target.value))}>
                <option value={0.03}>很高</option><option value={0.1}>标准</option><option value={0.2}>较低</option><option value={0.35}>低</option>
              </select>
            </Opt>
          </div>
        </Card>
        <Card className="grow-card" title="页面">
          <div className="pagelist" style={{ overflow: 'auto' }}>
            {imgState.diffs.map((d, i) => (
              <button key={i} onClick={() => document.getElementById(`page-${i}`)?.scrollIntoView({ behavior: 'smooth', block: 'start' })}>
                <span><span className="dot" style={{ background: d.pct < 0.001 ? 'var(--feather)' : 'var(--cardinal)' }} />第 {i + 1} 页</span>
                <span style={{ color: 'var(--text-3)' }}>{d.aMissing || d.bMissing ? '缺页' : d.pct < 0.001 ? '相同' : `${d.pct < 0.01 ? '<0.01' : d.pct.toFixed(2)}%`}</span>
              </button>
            ))}
          </div>
          <div className="progress-label" style={{ fontSize: 12, marginTop: 10, fontWeight: 700, whiteSpace: 'normal', lineHeight: 1.5 }}>Word 和 TXT 会先在本机排成页面再比较，分页可能和 Word 略有不同。</div>
        </Card>
      </>
    );
  }
  ...
  return (
    <div className={`app shell ${aside ? '' : 'no-aside'}`}>
      <nav className="nav">
        ...
        {MODES.map((m, i) => {
          ...
          return (
            <button key={i} className={`nav-item ${tab === i ? 'on' : ''}`} onClick={() => setTab(i)} title={kbd(`mod+${i + 1}`)}>
              {m.icon}<span>{m.label}</span>{n !== undefined && <span className="count">{n}</span>}
            </button>
          );
        })}
        <div className="spacer" />
        <div className="nav-foot">
          <button className="nav-item" onClick={clearAll}><I.IconPlusDoc /><span>新建比较</span></button>
          <button className="nav-item" onClick={() => setShowKeys(true)}><I.IconKeyboard /><span>快捷键</span></button>
          <button className="nav-item" onClick={() => setTheme(theme === 'dark' ? 'light' : 'dark')}>{theme === 'dark' ? <I.IconSun /> : <I.IconMoon />}<span>{theme === 'dark' ? '浅色模式' : '深色模式'}</span></button>
        </div>
      </nav>
      ...
      <div className="center">
        <div className="center-head drag">
          ...
          <button className="vs-chip" title={`交换左右（${kbd('mod+shift+S')}）`} onClick={swap}><I.IconSwap /></button>
        ...
        {changedSides.length > 0 && (
          <div className="banner">
            <I.IconRefresh />
            <span className="grow">「{changedSides.map((s) => docs[s]?.file.name).join('」和「')}」刚刚在磁盘上被修改了</span>
            <button className="btn sm blue" onClick={reloadChanged}>重新比较</button>
            <button className="btn sm plain" onClick={() => setChangedSides([])}>忽略</button>
          </div>
        )}
          ...
          <div className="main">
            ...
            {tab === 2 && changes.length > 0 && (
              <div className={`review-bar ${feedback || ''}`}>
                {feedback ? (
                  <div className="info"><span className="fb">{feedback === 'ok' ? <><I.IconCheck />已接受，保留新版本</> : <><I.IconX />已拒绝，恢复原文</>}</span></div>
                ) : cur ? (
                  <div className="info">
                    <span className="t"><span className={`tag ${cur.kind}`}>{KIND_LABEL[cur.kind]}</span>第 {curIdx + 1} / {changes.length} 处{decisions[cur.id] && <span className={`dec ${decisions[cur.id]}`}>{decisions[cur.id] === 'a' ? '· 已接受' : '· 已拒绝'}</span>}</span>
                    <span className="d">{cur.kind === 'mod' ? `“${short(cur.before, 40)}” → “${short(cur.after, 40)}”` : cur.kind === 'ins' ? `新增：${short(cur.after, 80)}` : cur.kind === 'del' ? `删除：${short(cur.before, 80)}` : cur.kind === 'fmt' ? `${short(cur.before, 40)}（${cur.note}）` : `移动：${short(cur.before, 80)}`}</span>
                  </div>
                ) : <div className="info"><span className="t">点文中任意一处修订开始审阅</span></div>}
                <span className="keys"><Kbd>R</Kbd>拒绝 <Kbd>A</Kbd>接受 <Kbd>J</Kbd><Kbd>K</Kbd>切换</span>
                <button className="btn red big" disabled={!cur} onClick={() => decide(selCid, 'r')}>拒绝</button>
                <button className="btn primary big" disabled={!cur} onClick={() => decide(selCid, 'a')}>接受</button>
              </div>
            )}
            {tab === 2 && doneOpen && (
              <div className="overlay-bg">
                ...
                <div className="done-card">
                  ...
                  <h2>审阅完成！</h2>
                  <div style={{ color: 'var(--text-2)', fontWeight: 700 }}>{changes.length} 处修订全部处理完毕</div>
                  <div className="done-stats">
                    <div className="done-stat a"><div className="h">接受</div><div className="v">{Object.values(decisions).filter((v) => v === 'a').length}</div></div>
                    <div className="done-stat r"><div className="h">拒绝</div><div className="v">{Object.values(decisions).filter((v) => v === 'r').length}</div></div>
                  </div>
                  <div className="done-acts">
                    <button className="btn primary big block" onClick={() => exportDocx(true)}>导出 Word 最终稿</button>
                    <button className="btn big block" onClick={exportPdf}>导出 PDF 修订对比</button>
                    <button className="btn plain block" onClick={() => setDoneOpen(false)}>回去再看看</button>
                  </div>
                </div>
              </div>
            )}
      ...
      {aside && <aside className="aside"><div className="aside-drag drag" />{aside}</aside>}
      {modals}
    </div>
  );
}
